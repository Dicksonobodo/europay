...
const AdminPanel = () => {
  ...
  const handleSuspend = async (u) => {
    ...
    await logAdminAction(
      userData.uid,
      userData.fullName,
      newState ? 'Suspended user' : 'Unsuspended user',
      u.fullName,
      `Email: ${u.email}`
    );
  ...
  // Joined Date
  // This remains unchanged because it is already working.
  const handleJoinedDateSave = async (u, value) => {
    if (!value) return;

    await updateUserJoinedDate(
      u.uid,
      new Date(value)
    );
  ...
  // Saves the selected transaction date to Firebase.
  const handleHistoryDateSave = async (tx) => {
    ...
    try {
      ...
      alert(
        `Failed to save transaction date: ${error.message}`
      );
    }
  };
  ...
  return (
    ...
    >
      ...
      <Modal
        ...
        title={
          historyUser
            ? `${historyUser.fullName}'s transactions`
            : 'User transactions'
        }
      >
        {historyLoading ? (
